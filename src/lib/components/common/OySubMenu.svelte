<script lang="ts">
  ...
  if(open == false){
    ...
    if(tabs){
      active = tabs[0];
      const indexTabLast = tabs.findIndex(item=>typeof item.is_last == 'boolean');
      if(indexTabLast < 0){
    ...
    <LayoutGrid class="p-0">
        ...
        <Cell spanDevices={{desktop: 3, mobile: 12}} class="p-40 pt-50">
            <TabBar {tabs} let:tab bind:active={active}>
                ...
                {#if tab.is_last == true}
                  <button on:click={()=>{goto(urlViewAll)}} class="mb-10 mdc-tab " role="tab" aria-selected="true" tabindex="0">
        ...
        <Cell spanDevices={{desktop: 4, mobile: 12}} class="p-50 pt-120 tab-content light">
            {#if active && active.name}
            <div>
                <div class="post-type mb-15 text-eyebrow">Most Popular</div>
                <h3 class="mb-40">{active.catalog}</h3>
                <Button type="button" variant="outlined" on:click={routerHelper.redirect(active.link)}>
