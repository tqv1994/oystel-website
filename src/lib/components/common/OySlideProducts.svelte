                    ...
                    {#if (i > 0)}
                        <Cell spanDevices={{desktop: 3, tablet: 2, phone: 1}}>
                            <div class="item-image">
                                <div class="thumbnail" on:mouseover={()=>{
                                        featuredImage = item;
                                    }} on:mouseleave={()=>{
                                        featuredImage = images[0];
                                    }}>
...
<style>
    ...
    @media (max-width: 1239px) and (min-width: 905px) {
        .slide-items :global(.mdc-layout-grid__inner) {
            grid-auto-columns: minmax(calc(1 / 12 * 100% - var(--mdc-layout-grid-gutter-tablet)), 1fr);
            grid-template-columns: repeat(auto-fill, minmax(calc(1 / 12 * 100% - var(--mdc-layout-grid-gutter-tablet)), 1fr));
        }
    }
