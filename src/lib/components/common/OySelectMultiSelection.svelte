<script>
  ...
  let oyMultiSelectWrap;
  onMount(() => {
    if (
      oyMultiSelectWrap.parentElement.classList.contains('multiple-limited')
    ) {
      showLimited = true;
    } else {
      showLimited = false;
...
<div
  class={`oy-multiSelect-wrap ${isShowAll ? 'show-all' : ''}`}
  use:clickOutside
  bind:this={oyMultiSelectWrap}
  on:click_outside={handleClickOutside}
>
  {#if showLimited && value.length > 1 && isShowAll === false}
    <div
