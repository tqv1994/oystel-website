export const documentHelper = {
  ...
  checkOffset: function (el: HTMLElement | null, elStop: HTMLElement | null) {
    if (el != null && elStop != null) {
      if (
        documentHelper.getRectTop(el) +
          document.body.scrollTop +
          el.offsetHeight >=
        documentHelper.getRectTop(elStop) + document.body.scrollTop - 10
      ) {
        const elTop = documentHelper.getRectTop(el);
        // el.classList.remove('fixed');
        const style = 'top: ' + elTop + 'px !important;';
      ...
      if (
        ...
        documentHelper.getRectTop(elStop) + document.body.scrollTop
      ) {
