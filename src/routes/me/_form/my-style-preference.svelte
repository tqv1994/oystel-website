  ...
  const checkLimitOfField = (index: number, value: string) => {
    setTimeout(() => {
      if (data[index]) {
        ...
        let selectedCurrent: string[] = beforeSelected.reduce(
          (acc: string[], item: string) => {
            const exist = data[index].preferences.find(
              (preference: string) => preference === item,
            );
            if (exist) {
              acc.push(exist);
            }
        ...
        if (index !== 1) {
          ...
          if (selectedCurrent.length > 1) {
            const idx = beforeSelected.indexOf(
              selectedCurrent[selectedCurrent.length - 2],
            );
            if (index > -1) {
              beforeSelected.splice(idx, 1);
              selected = beforeSelected;
          ...
          {#each type.preferences as option}
            <FormField>
              <Checkbox
                bind:group={selected}
                value={option}
                on:click={(e) => checkLimitOfField(index, option)}
              />
